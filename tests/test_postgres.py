...
from starz.db.store import Store, load, utc
...
class PostgresTests(unittest.TestCase):
    ...
    def snapshot(self):
        with Session(self.db) as session:
            empire = session.get(m.Empire, self.store.empire_id)
            return load(session, empire).to_dict()

    def restart(self):
        self.db.dispose()
        restarted = Store(self.db, self.catalog, seed='ignored-after-bootstrap')
        restarted.bootstrap(now=9999)
        self.assertEqual(restarted.empire_id, self.store.empire_id)
    ...
    def test_bootstrap_idempotent_and_concurrent(self):
        ...
        self.assertEqual(results, [self.store.empire_id] * 2)
    ...
    def test_full_flow_reload_offline_and_two_trips(self):
        job = self.store.run(lambda e: e.build('processor', now=e.state.last_updated), now=1000)
        pending = self.snapshot()
        self.assertEqual(len(pending['construction']), 1)
        self.restart()
        self.assertEqual(pending, self.snapshot())
        self.store.run(lambda e: None, now=1060)
        state = self.snapshot()
        ...
        research = self.store.run(lambda e: e.research('orbital_engineering', now=e.state.last_updated), now=1060)
        self.store.run(lambda e: None, now=1070)
        self.assertEqual(self.snapshot()['research']['remaining_work'], 35)
        self.restart()
        self.store.run(lambda e: None, now=research['complete_at'])
        self.assertEqual(self.snapshot()['research']['completed_at']['orbital_engineering'], 1105)
        job = self.store.run(lambda e: e.build('orbital_shipyard', now=e.state.last_updated), now=1105)
        ship = self.store.run(lambda e: e.build_ship(now=e.state.last_updated), now=job['complete_at'])
        state = self.snapshot()
        self.assertEqual(state['ships'][0]['id'], ship['id'])
        x, y = state['system_x'], state['system_y']
        first = self.store.run(lambda e: e.send_fleet(x+1, y, 'chemical_drive', 'NORMAL', now=e.state.last_updated), now=ship['ready_at'])
        self.restart()
        self.store.run(lambda e: None, now=first['arrival_at'])
        state = self.snapshot()
        self.assertEqual(state['fleets'][0]['status'], 'ARRIVED')
        self.assertEqual(state['fleets'][0]['x'], x+1)
        second = self.store.run(lambda e: e.send_fleet(x+2, y, 'chemical_drive', 'NORMAL', now=e.state.last_updated, fleet_id=first['id']), now=first['arrival_at'])
        self.assertEqual(second['preview']['distance'], 1)
        self.assertEqual(second['preview']['fuel_cost'], 10)
        self.assertEqual(second['id'], first['id'])
        self.restart()
        self.store.run(lambda e: None, now=second['arrival_at'])
        state = self.snapshot()
    ...
    def test_empty_bootstrap_concurrent(self):
        ...
        self.store.empire_id = results[0]
    ...
    def test_insufficient_fuel_rolls_back_advance_and_order(self):
        self.store.run(lambda e: e.state.districts.update(orbital_shipyard=1), now=1000)
        ship = self.store.run(lambda e: e.build_ship(now=e.state.last_updated), now=1000)
        self.store.run(lambda e: e.state.stocks.update(ion_fuel=0), now=ship['ready_at'])
        before = self.snapshot()
        with self.assertRaises(DataValidationError):
            self.store.run(lambda e: e.send_fleet(before['system_x']+1, before['system_y'], 'chemical_drive', 'NORMAL', now=e.state.last_updated), now=ship['ready_at']+10)
        self.assertEqual(before, self.snapshot())
    ...
    def test_rollback_after_mutation_and_database_failure(self):
        ...
        with self.assertRaises(DataValidationError):
            self.store.run(fail, now=1010)
        self.assertEqual(before, self.snapshot())
        def invalid(e):
            e.state.stocks['raw_ore'] = -1
        with self.assertRaises(IntegrityError):
            self.store.run(invalid, now=1010)
        self.assertEqual(before, self.snapshot())

    def test_concurrent_construction_cannot_double_spend(self):
        self.store.run(lambda e: e.state.stocks.update(refined_alloy=22, components=6), now=1000)
        barrier = Barrier(2)
        def build():
            barrier.wait(timeout=10)
            try:
                self.store.run(lambda e: e.build('processor', now=e.state.last_updated), now=1000)
                return 'success'
    ...
    def test_concurrent_shipyard_slot(self):
        ...
        def build():
            barrier.wait(timeout=10)
            try:
                self.store.run(lambda e: e.build_ship(now=e.state.last_updated), now=1000)
                return 'success'
            except DataValidationError:
                return 'rejected'
        with ThreadPoolExecutor(2) as pool:
    ...
    def test_database_unique_ownership(self):
        ...
        ship = self.store.run(lambda e: e.build_ship(now=e.state.last_updated), now=1000)
        state = self.snapshot()
        fleet = self.store.run(lambda e: e.send_fleet(state['system_x']+1, state['system_y'], 'chemical_drive', 'NORMAL', now=e.state.last_updated), now=ship['ready_at'])
        from uuid import UUID
        with self.assertRaises(IntegrityError), Session(self.db) as session, session.begin():
            other = m.Fleet(id=uuid4(), empire_id=self.store.empire_id, name='Other', x=0, y=0, destination_x=1, destination_y=0, status='TRANSIT', departure_at=utc(1030), arrival_at=utc(1200), propulsion_id='chemical_drive', mode='NORMAL', fuel_cost=10)
            session.add(other)
            session.flush()
            session.add(m.FleetShip(fleet_id=other.id, ship_id=UUID(ship['id']), empire_id=self.store.empire_id))
