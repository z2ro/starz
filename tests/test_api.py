        ...
        async def smoke():
            ...
            with patch.object(api.app.state, 'store', MemoryService(), create=True):
                ...
                async with httpx.AsyncClient(transport=httpx.ASGITransport(app=api.app), base_url='http://test') as client:
                    ...
                    galaxy = await client.get('/api/galaxy?radius=1')
                    self.assertEqual(galaxy.status_code, 200)
                    self.assertEqual(len(galaxy.json()['systems']), 9)
                    self.assertEqual(sum(item['home'] for item in galaxy.json()['systems']), 1)
                    home = next(item for item in galaxy.json()['systems'] if item['home'])
                    ...
                    self.assertEqual((await client.get('/api/galaxy?center_x=1')).status_code, 422)
                    self.assertEqual((await client.get('/api/galaxy?radius=4')).status_code, 422)
                    ...
                    self.assertEqual(response.json()['fleets'][0]['mission'], 'MOVE')
                    centered = await client.get(f'/api/galaxy?center_x={x + 2}&center_y={y}&radius=1')
                    destination = next(item for item in centered.json()['systems'] if (item['x'], item['y']) == (x + 2, y))
                    self.assertEqual(destination['knowledge_level'], 'UNKNOWN')
                    self.assertNotIn('planet', destination)
                    payload.pop('ship_id')
                    payload.update(fleet_id=first['id'], target_x=x + 3, mission='SURVEY')
                    previews = await post('/api/travel-preview', payload)
                    ...
                    self.assertEqual(response.json()['fleets'][0]['mission'], 'SURVEY')
                    surveyed = await client.get(f'/api/galaxy?center_x={x + 3}&center_y={y}&radius=1')
                    destination = next(item for item in surveyed.json()['systems'] if (item['x'], item['y']) == (x + 3, y))
                    self.assertEqual(destination['knowledge_level'], 'SURVEYED')
                    self.assertIn('planet', destination)
                    ...
                    invalid = await client.post('/api/travel', json={**payload, 'mission': 'SCAN'})
                    self.assertEqual(invalid.status_code, 422)
        asyncio.run(smoke())
