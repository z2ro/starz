# STARZ — Frotas Imperial Command

Pranchas e documentação naval das rodadas Ship Visual Language V1/V2 e Capital Ship Visual Language V3.

## Naves

| Nave / classe | Classificação | Função | Prancha |
|---|---|---|---|
| [Horizon](HORIZON.md) | Exploration Corvette | Exploração, reconhecimento e levantamento de sistemas. | [PNG](assets/horizon-concept.png) |
| [Wayfarer](WAYFARER.md) | Freighter | Transporte, logística e sustentação da expansão. | [PNG](assets/wayfarer-concept.png) |
| [Vanguard](VANGUARD.md) | Frigate | Escolta, patrulha, proteção de rotas e combate equilibrado. | [PNG](assets/vanguard-concept.png) |
| [Sentinel](SENTINEL.md) | Destroyer | Patrulha pesada, defesa, escolta reforçada e sustentação da linha. | [PNG](assets/sentinel-concept.png) |
| [Odyssey](ODYSSEY.md) | Science Cruiser | Ciência de campo, exploração avançada, observação e análise. | [PNG](assets/odyssey-concept.png) |
| [Aegis](AEGIS.md) | Cruiser | Presença de linha, combate sustentado, escolta pesada e projeção regional. | [PNG](assets/aegis-concept.png) |
| [Spearhead](SPEARHEAD.md) | Battlecruiser | Ofensiva, ruptura de linha e assalto frontal. | [PNG](assets/spearhead-concept.png) |
| [Leviathan](LEVIATHAN.md) | Battleship | Combate pesado de linha, guerra sustentada, bateria pesada e engajamento de cerco. | [PNG](assets/leviathan-concept.png) |
| [Atlas](ATLAS.md) | Carrier | Suporte de frota, operações de strike craft, comando e projeção de longo alcance. | [PNG](assets/atlas-concept.png) |
| [Dominion](DOMINION.md) | Dreadnought | Domínio militar estratégico, combate de grande escala e plataforma pesada de comando. | [PNG](assets/dominion-concept.png) |
| [Stargrave-class](STARGRAVE-CLASS.md) | Titan | Supercapital estratégico, centro da frota de endgame e plataforma extrema de comando e projeção. | [PNG](assets/stargrave-class-concept.png) |

O complemento do Atlas possui fichas próprias: [Interceptor](INTERCEPTOR.md), [Fighter](FIGHTER.md) e [Bomber](BOMBER.md). São unidades subordinadas, além das 11 classes principais.

## Guias

- [Capital Ship Visual Guide](CAPITAL-SHIP-VISUAL-GUIDE.md)
- [Fleet Scale Guide](FLEET-SCALE-GUIDE.md)
- [Ship Asset Manifest](SHIP-ASSET-MANIFEST.md)
- [Codex Ship Handoff](CODEX-SHIP-HANDOFF.md)

## Pranchas coletivas

- [Escala e funções da frota V2](assets/fleet-scale-v2.png)
- [Shipyard, Fleet Operations e Planet View V2](assets/fleet-contexts-v2.png)
- [STARZ Imperial Fleet V3 — 11 classes](assets/imperial-fleet-v3.png)
- [Capital ships: Shipyard, construção, operações e órbita V3](assets/capital-contexts-v3.png)

## Como usar as referências

As imagens são as pranchas originais geradas nesta tarefa, preservadas sem redesenho. A ficha individual de cada nave é a referência de identidade do casco. Os quadros coletivos ilustram composição e contexto; contêm variações de geometria, sobretudo no Stargrave, no Atlas e no Odyssey, e não substituem os modelos individuais.

O Spearhead usa a última revisão, com SPEARHEAD no casco e no detalhe de materiais. As versões anteriores dessa prancha não foram incluídas.

Escala visual não equivale a dimensão física canônica. Os PNGs são concept sheets compostos; SVGs, layers separados e modelos 3D ainda não foram exportados. As vistas técnicas precisam de conferência geométrica antes da modelagem.

[Voltar ao pacote Imperial Command V2](../README.md)

