...
type Hull = Content & { mass: number; crew: number; duration: number; compatible_propulsion: string[] };
...
type State = {
  ...
  fleets: Array<{ id: string; name: string; status: 'ARRIVED' | 'TRANSIT'; x: number; y: number; destination_x: number; destination_y: number; ship_ids: string[]; eta: number; propulsion: string; mode: string; fuel_cost: number; departure_at: number; arrival_at: number }>;
...
type GalaxySystem = { id: string; name: string; x: number; y: number; distance: number; home: boolean; star: { stellar_class: string; luminosity: number; activity: number }; planet: { name: string; gravity: number; temperature: number; water: number; radiation: number } };
type Galaxy = { center: number[]; radius: number; systems: GalaxySystem[] };
...
const label = (id: string) => [...(content?.resources ?? []), ...(content?.fuels ?? []), ...(content?.districts ?? []), ...(content?.technologies ?? []), ...(content?.propulsion ?? [])].find(item => item.id === id)?.name ?? id.replaceAll('_', ' ');
...
async function load(showBusy = false): Promise<void> { if (showBusy) { busy = true; if (current) render(); } current = await request<State>('/api/state'); busy = false; render(); }
