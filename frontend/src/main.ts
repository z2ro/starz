type State = {
  system: { name: string; x: number; y: number; star: { stellar_class: string; luminosity: number }; planet: Record<string, unknown> };
  stocks: Record<string, number>;
  capacities: { energy_generation: number; energy_consumption: number; industrial: number };
  population: { total: number; available: number; capacity: number };
  districts: Record<string, number>;
  research: { active: string | null; complete_at: number | null; completed: string[] };
  fleets: Array<{ id: string; name: string; status: string; x: number; y: number; destination_x: number; destination_y: number; ship_ids: string[]; eta: number; propulsion: string }>;
  ships: Array<{ id: string; propulsion_id: string; ready_at: number }>;
  travel_modes: Array<{ id: string; name: string }>;
  notices: string[];
};

const esc = (value: unknown) => String(value ?? '').replace(/[&<>"']/g, (c) => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]!));
const fmt = (value: unknown) => typeof value === 'number' ? value.toFixed(value % 1 ? 1 : 0) : esc(value);
...
function routeControls(s: State) {
  ...
  if (!subjects.some(subject => subject.value === selectedSubject && !subject.disabled)) selectedSubject = subjects.find(subject => !subject.disabled)?.value ?? '';
  if (!s.travel_modes.some(mode => mode.id === selectedMode)) selectedMode = s.travel_modes[0]?.id ?? '';
...
function render(s: State) {
  ...
  app.innerHTML = `<header><div class="brand">STAR<span>Z</span></div><div class="hud"><b>MATÉRIA</b> ${fmt(s.stocks.raw_ore)} <b>LIGA</b> ${fmt(s.stocks.refined_alloy)} <b>COMP</b> ${fmt(s.stocks.components)} <b>ENERGIA</b> ${fmt(energy)} / ${fmt(s.capacities.energy_generation)} <b>POP</b> ${fmt(s.population.total)}</div></header>
    ...
    <div class="columns"><section class="panel"><h2>Desenvolvimento</h2><div class="rows">${Object.entries(s.districts).map(([id, level]) => `<div><span>${esc(id.replaceAll('_',' '))}</span><b>nível ${level}</b></div>`).join('')}</div><button class="primary" data-action="build" data-id="processor">Construir processador</button><button data-action="build" data-id="orbital_shipyard">Construir estaleiro orbital</button></section><section class="panel"><h2>Pesquisa</h2><p>${s.research.active ? `Em andamento: <b>${esc(s.research.active)}</b>` : 'Nenhuma pesquisa em andamento.'}</p><button class="primary" data-action="research" data-id="orbital_engineering">Iniciar engenharia orbital</button></section><section class="panel"><h2>Frotas</h2>${s.fleets.length ? s.fleets.map((f) => `<div class="fleet"><b>${esc(f.name)}</b><span>${esc(f.status)} · ${f.status === 'TRANSIT' ? 'Origem' : 'Posição'} ${f.x}:${f.y} → ${f.destination_x}:${f.destination_y} · ETA ${f.eta}s</span></div>`).join('') : '<p class="muted">Nenhuma frota em trânsito.</p>'}<button class="primary" data-action="build-ship">Montar scout</button><button data-action="refresh">Atualizar estado</button></section></div>
  ...
  app.querySelectorAll<HTMLElement>('[data-action]').forEach((button) => button.onclick = () => { void act(button.dataset.action!, button.dataset.id); });
  app.querySelectorAll<HTMLInputElement | HTMLSelectElement>('#travel-subject, #travel-mode, #target-x, #target-y').forEach(control => control.onchange = () => {
    selectedSubject = document.querySelector<HTMLSelectElement>('#travel-subject')!.value;
    selectedMode = document.querySelector<HTMLSelectElement>('#travel-mode')!.value;
    destination = { x: document.querySelector<HTMLInputElement>('#target-x')!.valueAsNumber, y: document.querySelector<HTMLInputElement>('#target-y')!.valueAsNumber };
    document.querySelector('#route-preview')!.textContent = 'Rota alterada. Compare novamente antes de enviar.';
  });
}

type TravelPreview = { origin: number[]; distance: number; eta_seconds: number; fuel_cost: number; heat: number; signature: number };

async function act(action: string, id?: string) {
  try {
    if (action === 'refresh') { await load(); return; }
    let body: Record<string, unknown> = action === 'build' || action === 'research' ? { id } : {};
    if (action === 'travel' || action === 'preview') {
      if (!destination || !Number.isInteger(destination.x) || !Number.isInteger(destination.y)) throw new Error('Informe coordenadas inteiras.');
      ...
      if (!subjectId) throw new Error('Selecione uma frota ou nave.');
    ...
    if (!response.ok) throw new Error(typeof result.detail === 'string' ? result.detail : 'Pedido inválido. Verifique os campos.');
    document.querySelector('#action-error')!.textContent = '';
    if (action === 'preview') {
      document.querySelector('#route-preview')!.innerHTML = Object.entries(result as Record<string, TravelPreview>).map(([name, value]) => `<span><b>${esc(name)}</b> origem ${value.origin.join(':')} · distância ${fmt(value.distance)} · ETA ${value.eta_seconds}s · combustível ${value.fuel_cost} · calor ${value.heat} · assinatura ${value.signature}</span>`).join('');
      return;
    }
    await load();
  } catch (error) {
...
async function load() {
  ...
  if (!response.ok) throw new Error('Não foi possível carregar o estado.');
  render(await response.json() as State);
}
